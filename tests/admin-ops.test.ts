...
describe("buildCostAlerts", () => {
  it("keeps only orgs over the threshold, sorted worst-first", () => {
    const alerts = buildCostAlerts([
      // Growth INR is ₹9,999 (≈ $110–120). $60 of AI cost on any plan under
      // ~$170 is over 35%; $0.10 on the same plan is not.
      { orgId: "a", orgName: "Hot", plan: "growth", currency: "INR", costMicroUsd30d: 60_000_000 },
  ...
  it("never alerts on unpriced (free/unknown) plans and survives junk currency", () => {
    const alerts = buildCostAlerts([
      ...
      { orgId: "d", orgName: "Junk", plan: "growth", currency: "XXX", costMicroUsd30d: 60_000_000 },
    ]);

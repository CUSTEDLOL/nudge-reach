...
function lastAudit() {
  return prisma.auditLog.create.mock.calls.at(-1)?.[0].data;
}
...
describe("setTrial", () => {
  ...
  it("0 days clears the trial", async () => {
    ...
    expect(prisma.org.update.mock.calls[0][0].data.trialEndsAt).toBeNull();
  });
});
...
describe("setSubscriptionStatus", () => {
  ...
  it("writes the new status and audits it", async () => {
    ...
    expect(res.ok).toBe(true);
    expect(prisma.org.update.mock.calls[0][0].data).toEqual({ subscriptionStatus: "active" });
...
describe("setLiveMode", () => {
  ...
  it("goes live once a number exists, and back to test any time", async () => {
    ...
    expect(prisma.org.update.mock.calls[0][0].data).toEqual({ simulated: false });
...
describe("setVoiceMinutes", () => {
  it("validates the range and treats null as 'plan default'", async () => {
    ...
    expect(res.ok).toBe(true);
    expect(prisma.org.update.mock.calls[0][0].data).toEqual({ voiceMinutesOverride: 250 });
...
describe("setSuspended", () => {
  ...
  it("suspends with a timestamp and lifts with null, auditing each", async () => {
    ...
    expect(prisma.org.update.mock.calls[0][0].data.suspendedAt).toBeInstanceOf(Date);
    ...
    expect(prisma.org.update.mock.calls[1][0].data).toEqual({ suspendedAt: null });
...
describe("feature overrides", () => {
  ...
  it("stores only sanitized keys and audits the diff", async () => {
    ...
    expect(res.ok).toBe(true);
    expect(prisma.org.update.mock.calls[0][0].data).toEqual({ featureOverrides: { publicApi: true } });

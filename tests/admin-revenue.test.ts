...
describe("aggregateMrr", () => {
  it("sums list prices per currency and plan, ordered by plan tier", () => {
    ...
    expect(byCurrency).toEqual([
      { currency: "INR", monthly: 30997, orgs: 3 },
      { currency: "SGD", monthly: 599, orgs: 1 },
    ]);
  });

  it("counts free and enterprise (contact-only) orgs at zero, and maps legacy ids", () => {
    ...
    expect(rows.find((r) => r.plan === "pro")?.monthly).toBe(159);
  });

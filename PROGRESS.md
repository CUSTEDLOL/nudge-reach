...
- Production schema release was applied additively. Before branch integration,
  `prisma db push` refused because the feature checkout did not yet contain
  `main`'s `Automation.source` and `AutomationRunStatus.CANCELLED` definitions;
  no data-loss override was used. Only the two approved `AcquisitionTrial`
  counters were added with idempotent SQL, the backfill updated one legacy
  website trial and zero file trials, and RLS remained enabled on all 48 public
  tables. After merging `main`, `db:push` confirmed the schema is fully in sync.
- Fresh release verification passed: 23 focused files / 321 tests; the combined
  branch passed 242 full test files / 1,895 tests, with the existing three

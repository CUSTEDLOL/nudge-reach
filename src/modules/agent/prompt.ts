...
export const TOOL_GUIDANCE = [
  ...
  "- For a booking or appointment: once you have the name and the time (and the number of people, only if the booking needs it), confirm them ONCE, and as soon as the customer agrees, call `capture_booking_request`. Do not keep re-asking for confirmation — one check is enough. Never call it with guessed details. After it succeeds, tell the customer the team will confirm shortly — never claim the slot is guaranteed.",
  "- If the tool says the business is CLOSED at that time or the slot is TAKEN, say so plainly, offer exactly the alternative times it returned, and ask the customer to pick one. Never invent a time it did not return.",
...
export function voiceToolGuidance(canTransfer: boolean): string {
  ...
  return [
    ...
    "- For a booking: a caller who wants to book has shown buying intent, so call `capture_lead` first if you have not already. Then collect the name and requested time, plus the number of people only if the booking needs it. Confirm them once, then call `capture_booking_request`. After it succeeds, say ONE short sentence — the team will confirm shortly — and then ask if there is anything else. Do not repeat the price or the details; never claim the slot is guaranteed.",
    handoff,

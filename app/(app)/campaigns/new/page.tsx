...
import { generateCampaignAction, type ActionResult } from "../actions";
...
export default function NewCampaignPage() {
  ...
  return (
    <div className="max-w-xl">
      <PageHeader
        title="New campaign"
        description="Add a photo of your product — that's it. We'll write the message, the offer and the buttons."
      />

      <form
        action={action}
        className="flex flex-col gap-5 rounded-2xl border border-black/5 bg-white p-6 shadow-soft"
      >
        ...
        <label className="block">
          ...
          <textarea
            name="description"
            rows={3}
            className="mt-2 w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm outline-none focus:border-brand-500"
        ...
        <button
          type="submit"
          disabled={pending}
          className="rounded-lg bg-brand-600 px-4 py-3 text-sm font-semibold text-white transition-colors duration-150 hover:bg-brand-700 disabled:opacity-60"
        >
          {pending
